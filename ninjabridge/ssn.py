...
class SsnClient:
    def __init__(self,url:str,session_id:str,relay_targets:tuple[str,...],logger:logging.LoggerAdapter,on_message:MessageHandler|None=None)->None:
    ...
    async def publish_discord(self,payload:dict[str,Any])->None:
        ...
        for target in self.relay_targets:await self.send_chat(target,to_relay_text(payload))
    async def _enqueue(self,command:dict[str,Any])->None:
        if self.queue.full():self.queue.get_nowait();self.queue.task_done();self.logger.warning('SSN queue full; discarded oldest command')
        await self.queue.put(command)
    async def _sender(self,socket:Any)->None:
        while not self.stopping:
            command=await self.queue.get()
            try:await socket.send(json.dumps(command,separators=(',',':')))
            except Exception:
                self.queue.task_done();await self._enqueue(command);raise
    ...
    async def _receiver(self,socket:Any)->None:
        async for raw in socket:
            ...
            if self.on_message and isinstance(data,dict):
                try:await self.on_message(data)
                except Exception:self.logger.exception('Failed to process SSN message')
        ...
        while not self.stopping:
            try:
                ...
                async with websockets.connect(self.url,open_timeout=15,ping_interval=25,ping_timeout=15,close_timeout=5) as socket:
                    await socket.send(json.dumps({'join':self.session_id,'in':4,'out':1}));attempt=0;self.logger.info('Connected to SSN channels 4→1')
                ...
                delay=min(30,2**attempt)+random.random()*.5;attempt+=1;self.logger.exception('SSN connection failed; retrying in %.1fs',delay);await asyncio.sleep(delay)
